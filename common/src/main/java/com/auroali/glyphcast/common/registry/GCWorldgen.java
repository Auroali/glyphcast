...
public class GCWorldgen {
    public static void init() {
        LifecycleEvent.SETUP.register(() ->
            BiomeModifications.addProperties((ctx, mutable) -> {
                mutable.getGenerationProperties().addFeature(GenerationStep.Decoration.UNDERGROUND_ORES, PLACED_CRYSTAL_ORE.get());
            })
        );
    ...
    public static final RegistrySupplier<PlacedFeature> PLACED_CRYSTAL_ORE = PLACED_FEATURES.register("crystal_ore_placed", () -> new PlacedFeature(Holder.direct(CRYSTAL_ORE.get()),
            List.of(CountPlacement.of(1), InSquarePlacement.spread(), HeightRangePlacement.triangle(VerticalAnchor.aboveBottom(0), VerticalAnchor.aboveBottom(30))
            )));
}
