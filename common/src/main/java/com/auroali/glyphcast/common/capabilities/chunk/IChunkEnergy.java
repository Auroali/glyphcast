...
public interface IChunkEnergy {
    ...
    static List<Fracture> getNearbyFractures(Level level, BlockPos pos, int range) {
        ...
        for (int cX = -range; cX <= range; cX++) {
            for (int cZ = -range; cZ <= range; cZ++) {
                ...
                builder.addAll(get(level.getChunk(cX + x, cZ + z))
    ...
    static Optional<Fracture> getFractureAt(Level level, BlockPos pos) {
        return get(level.getChunk(SectionPos.blockToSectionCoord(pos.getX()), SectionPos.blockToSectionCoord(pos.getZ())))
                .flatMap(e ->
                        e.getFractures()
                        .stream()
    ...
    @ExpectPlatform
    static Optional<IChunkEnergy> get(LevelChunk chunk) {
