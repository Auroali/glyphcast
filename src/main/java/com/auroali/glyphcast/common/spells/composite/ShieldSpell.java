    ...
    protected void run(IContext ctx, int usedTicks) {
        ...
        ctx.level().getEntities(ctx.player(), aabb, e -> true)
                .forEach(e -> {
                    ...
                    GCNetwork.sendToNear(ctx.level(), e.position(), 16, new SpawnParticlesMessage(GCParticles.MAGIC_AMBIENCE.get(), 0.1, 6, e.position().add(0, e.getEyeHeight() / 2, 0), pushVec.normalize(), pushVec.length()));
